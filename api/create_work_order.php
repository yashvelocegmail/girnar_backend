...
if(empty($_FILES))
{
    ...
    //print_r($purchase_order);die;
    $get_purchase_order = $items->get_purchase_order();
    while ($row = mysqli_fetch_assoc($get_purchase_order)) {
        //print_r($row);die;
        $purchase_order = $row["purchase_order"];
    }
    $get_purchase_order_from_work_order = $items->get_purchase_order_from_work_order();
    $items->work_order=$purchase_order."-WO-".strval($get_purchase_order_from_work_order->num_rows+1);
    ...
    if($create_third_party)
    {
        ...
        $response_arr["messsage"] = "Work Order created successfully";
    ...
    else
    {
        ...
        $response_arr["messsage"]= "Work Order could not be created.";
        echo json_encode($response_arr);
    }    
}
else
{
    ...
    $create_third_party=$items->create_work_order_with_files();
    $response_arr = array();
    if($create_third_party)
    {
        http_response_code(200);
